		...
		for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
			...
			try {
				const { data: batchData, error: batchError } =
					await resend.batch.send(batchPayloads);
				// Debug logging
				console.log(
...
const getInstallationPackage = (productName) => {
...
function generateEmailHtml(data) {
	...
	if (data.phases === "1") {
		selectedProducts.push("AMINA 1 - 7.4kW");
	...
	const productSections = selectedProducts
		.map((product, index) => {
			...
			const installationPrice = data.needsInstallation
      ...
      <div class="product-section" style="margin-bottom: 24px; background-color: #f3f4f6; padding: 10px; border-radius: 12px; border: 2px solid #e5e7eb;">
        ...
        <div style="margin-top: 20px; background-color: white; padding: 8px; border-radius: 8px; border: 2px solid #0071e3;">
          ...
          <div style="text-align: center; margin-top: 16px;">
            <a href="${getProductUrl(product)}" class="button-link" style="display: inline-block; background: linear-gradient(135deg, #0071e3 0%, #005bb5 100%); color: #ffffff; padding: 10px 24px; border-radius: 8px; text-decoration: none; font-size: 14px; font-weight: 600; box-shadow: 0 4px 12px rgba(0, 113, 227, 0.3);">Megnézem</a>
          </div>
        </div>
      </div>
    `;
